...
/**
 * Clase GeneradorPruebas2.
 */
public class GeneradorPruebas2 {
	...
	/** Incluye las pérdidas de la ciudad. */
	private ArrayList<String> perdidas;
	 ...
	 * @param fallosFlujo
	 */
	public GeneradorPruebas2(int avenidas, int calles, ArrayList<String> fallosPresion, ArrayList<String> fallosFlujo, ArrayList<String> perdidas) {
	...
	private StringBuilder generarFlujo(int avenidas, int calles) {
		...
		int perdida = 0;
		boolean errorAqui = false;
		boolean errorAquiPerdida = false;
		boolean errorFlujo = false;
		if (!(fallosFlujo.isEmpty() || fallosFlujo == null)) errorFlujo = true;
		boolean hayPerdidas = false;
		if (!(perdidas.isEmpty() || perdidas == null)) hayPerdidas = true;
		...
		for (int i = avenidas; i > 0; i -= 2) {
			for (int j = calles; j >= 3; j--) {
				...
				s.append(sFlujos.delete(sFlujos.length() - 1, sFlujos.length())).append(" ").append("M-").append(sMedia.delete(sMedia.length() - 1, sMedia.length()));
				
				//Inserta porcentaje de error si hay perdidas en los contadores
				if(hayPerdidas) {
					for (String elemento : perdidas) {
						String[] partes = elemento.split(" ");
						errorAvenida = Integer.parseInt(partes[0].substring(1, partes[0].indexOf('C')));
						errorCalle = Integer.parseInt(partes[0].substring(partes[0].indexOf('C') + 1));
						perdida = Integer.parseInt(partes[1]);
						if(i == errorAvenida && j == errorCalle) {
							errorAquiPerdida = true;
							break;
						}
					}
				}
				if(errorAquiPerdida) {
					s.append(" ").append(perdida).append("%, ");
				} else {
					s.append(" 0%, ");
				}
